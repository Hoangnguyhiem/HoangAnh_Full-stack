...
const DetailPage = (props: Props) => {
  ...
  // Thay đổi tab size khi chọn màu
  useEffect(() => {
    const tabs = document.querySelectorAll(".tab_color");
    const tabsItem = document.querySelectorAll(".tab_color-size");

    tabs.forEach((el: any) => el.addEventListener("click", function (e: any) {
      const idTabel = e.target.dataset.tab

      tabs.forEach((el) => el.classList.remove("border-black"))
      e.target.classList.add("border-black");

      tabsItem.forEach((el) => {
        el.classList.remove("flex");
        el.classList.add("hidden");
        if (el.getAttribute("data-tab") === idTabel) {
    ...
    if (colors) {
      ...
      if (colorData) {
        ...
        if (availableSize) {
          ...
          setSelectedSize(''); // No available size
        }
      }
    }
  }, [colors, data]);
          ...
          <div className="pc:px-[48px] lg:flex lg:flex-wrap">
            ...
            {/* <div className="lg:w-[55%] lg:px-[5%] lg:mb-[30px] lg:flex lg:gap-4 lg:h-screen">
              ...
              <div className="relative lg:w-[86%] lg:order-2">
                ...
                <div className="">
                  <div className="w-[100%] relative">
                    ...
                    {data?.data.attributes.map((item: any) => (
                      <div className={`${colors === item.color ? "" : "hidden"}`}>
                        {
                          item.image.map((value: any, index: any) => (
                            <div className={`${index===0 ? "" : "hidden"}pt-[124%] bg-cover bg-no-repeat bg-center`} style={{ 'backgroundImage': `url(${value})` }}>
                              <div className="absolute top-[50%] translate-y-[-50%] *:text-black flex w-[100%] justify-between">
                                <button className='w-[40px] h-[40px] flex justify-center items-center'>
                                  <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 24 46" fill="none"><path d="M22.5 43.8335L1.66666 23.0002L22.5 2.16683" stroke="black" stroke-width="2" stroke-linecap="square"></path></svg>
                                </button>
                                <button className='w-[40px] h-[40px] flex justify-center items-center'>
                                  <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 24 46" fill="none"><path d="M1.66675 2.1665L22.5001 22.9998L1.66675 43.8332" stroke="black" stroke-width="2" stroke-linecap="square"></path></svg>
                                </button>
                              </div>
                            </div>
                          ))}
                ...
                <div className="absolute bottom-[15px] left-[50%] translate-x-[-50%] lg:hidden">
                  <button className='w-[8px] h-[8px] rounded-[50%] border-[#808080] border-[1px] mx-[3.5px] bg-black'>
                    <span></span>
                  </button>
                  <button className='w-[8px] h-[8px] rounded-[50%] border-[#808080] border-[1px] mx-[3.5px]'>
                    <span></span>
                  </button>
                  <button className='w-[8px] h-[8px] rounded-[50%] border-[#808080] border-[1px] mx-[3.5px]'>
                    <span></span>
              ...
              {data?.data.attributes.map((item: any) => (
                <div className={`${colors === item.color ? "lg:flex" : "lg:hidden"} hidden lg:w-[14%]  lg:flex-col lg:overflow-auto lg:order-1 lg:gap-4 lg:pr-[8px] lg:h-[96%] scrollbar`}>

                  {
                    item.image.map((value: any, index: any) => (
                      <div key={index + 1} className="pt-[120%] bg-cover bg-no-repeat bg-center" style={{ 'backgroundImage': `url(${value})` }}></div>
                    ))
                  }
                </div>
              ))}
            ...
            <div className="lg:w-[55%] lg:px-[5%] lg:mb-[30px] lg:flex lg:gap-4 lg:h-screen">
              <div className="relative lg:w-[86%] lg:order-2">
                ...
                <div className="absolute bottom-[15px] left-[50%] translate-x-[-50%] lg:hidden">
                  {data?.data.attributes[0].image.map((_: any, dotIndex: any) => (
                    <button
                      key={dotIndex}
                        ...
                        {item.sizes.map((value: any, index: any) => (
                          <>
                            <input
                              ...
                              checked={sizes.size === value.size}
                            />

                            {value.stock === 0 ?
                              <label htmlFor={value.size} key={index + 1} className="flex pointer-events-none bg-[#F8F8F8] text-[#D0D0D0]">{value.size}</label> :
                              <label htmlFor={value.size} key={index + 1} className={`flex ${sizes.size === value.size ? "bg-black text-white" : "bg-white text-black"}`}>{value.size}</label>
                            }
                          </>
                        ))}
                      </div>
